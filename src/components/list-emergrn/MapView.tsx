...
import { ScrollView, StyleSheet, Text, TouchableOpacity, View, type DimensionValue } from 'react-native';
import Svg, {
  Circle,
  G,
  Image as SvgImage,
...
const checkPinMatch = (pin: typeof BUILDINGS[0], report: Report) => {
  ...
  const pinWords = pinName.split(' ').filter(w => w.length >= 2);
  const reportWords = reportLoc.split(' ').filter(w => w.length >= 2);
  
  for (const pWord of pinWords) {
    if (reportWords.includes(pWord)) return true;
  }
  
  if (pin.bid === 8 && (reportLoc.includes('هندسه') || reportLoc.includes('تقنيه') || reportLoc.includes('اي تي') || reportLoc.includes('it'))) return true;
  if (pin.bid === 22 && reportLoc.includes('طب')) return true;
  if (pin.bid === 20 && reportLoc.includes('مكتب')) return true;
  if (pin.bid === 14 && reportLoc.includes('ساحه')) return true;
        ...
        <ScrollView style={styles.mapScroll} contentContainerStyle={styles.mapScrollContent}>
          <Svg width="100%" viewBox={`0 0 ${SVG_W} ${SVG_H}`} style={styles.svg}>
            ...
            {reports.map((report, index) => {
              let matchedPin = BUILDINGS.find(pin => checkPinMatch(pin, report));
              if (!matchedPin) {
                matchedPin = { bid: -1, name: 'غير محدد', cx: 30, cy: 15 };
              }

              const priorReportsSamePin = reports.slice(0, index).filter(r => {
                let p = BUILDINGS.find(b => checkPinMatch(b, r));
                if (!p) p = { bid: -1, name: 'غير محدد', cx: 30, cy: 15 };
              ...
              return (
                <G key={report.id} onPress={() => onMarkerPress(isSelected ? null : report.id)}>
                  <Circle cx={cx} cy={cy} r="9" fill={color} opacity="0.2" pointerEvents="none"/>
                  <Circle cx={cx} cy={cy} r="5" fill={color} pointerEvents="none"/>
                  <SvgText x={cx} y={cy + 2} textAnchor="middle"
                    fontSize="6" fontWeight="700" fontFamily="System" fill="#fff" pointerEvents="none">!</SvgText>
                  {isSelected && (
                    <Circle cx={cx} cy={cy} r={12} fill="none" stroke={color} strokeWidth="2" pointerEvents="none"/>
                  )}
...
function PinPopup({ report, leftPercent, topPercent, onClose }: PinPopupProps) {
  ...
  return (
    ...
    >
      <View style={[styles.tail, showAbove ? styles.tailBottom : styles.tailTop]} />
      <View style={styles.popupHeader}>
        <Text style={styles.popupType}>{getTypeLabel(report.type as EmergencyType)}</Text>
        <TouchableOpacity onPress={onClose} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
          <Text style={styles.popupClose}>✕</Text>
        </TouchableOpacity>
      </View>
      <Text style={styles.popupLocation} numberOfLines={1}>📍 {report.location}</Text>
      <Text style={styles.popupDesc} numberOfLines={2}>{report.description}</Text>
